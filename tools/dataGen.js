/**
 * Data Generator for QuickFill ML
 * Generates variations of job application questions mapped to profile keys.
 */
...
function generateVariations() {
  ...
  for (const [key, variants] of Object.entries(DATA_MAP)) {
    variants.forEach(text => {
      ...
      // Add variations (extra punctuation, casing, etc. implicitly handled by lowercasing)
      trainingData.push({ input: text.toLowerCase() + '?', output: key });
      trainingData.push({ input: text.toLowerCase() + ':', output: key });
      trainingData.push({ input: 'please enter ' + text.toLowerCase(), output: key });
      trainingData.push({ input: 'select ' + text.toLowerCase(), output: key });
    });
  }
  return trainingData;
}
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { generateVariations, DATA_MAP };
} else {
